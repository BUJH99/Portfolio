...
[MODULE_INFO_START]
Name: ExpSum
Role: Exponential approximation stage for frame sum
Summary:
  - Approximates exp(x) for the accumulation path
...
module ExpSum(
    ...
    function [15:0] fnAbsSatQ78;
        ...
        reg [LP_ABS_W-1:0] absValue;
    begin
        if (iValue[15])
            absValue = {1'b0, ~iValue} + {{LP_DATA_W{1'b0}}, 1'b1};
        else
            absValue = {1'b0, iValue};

        if (absValue > {1'b0, LP_Q78_MAX_MAG})
            fnAbsSatQ78 = LP_Q78_MAX_MAG;
        else
            fnAbsSatQ78 = absValue[LP_DATA_W-1:0];
    end
    endfunction
    ...
    wire [LP_DATA_W-1:0] absValue      = fnAbsSatQ78(iData);
    wire                 highCoeffEn   = (absValue[14:12] == 3'b000);
